...
def prediction_ratio_check():
    ...
    log_entry = (
        ...
        f"%of 2 = {str(value_counts.get(0, 0))}"
    )
...
def input_features_check():
    ...
    Logs the status to the console, ensuring the pipeline doesn't break due to
    missing columns.
    ...
    OUTPUT:
        Logs the presence of all input columns.
          - 'All the models input are present' if complete.
          - 'Some of the models inputs are missing' otherwise.
    ...
    # Check if the set of columns matches the expected features.
    if set(inference_df.columns) == set(ONE_HOT_ENCODED_FEATURES):
        print('All the model input are present')
    else:
        print('Some of the model inputs are missing')
    connection.close()
